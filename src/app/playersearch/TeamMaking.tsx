...
const TeamMaking = () => {
  ...
  const { user, loading: userLoading } = useAuth() as { user: any; loading: boolean };
  ...
  useEffect(() => {
    if (!sports) return;
    const options = sports.map((sport: any) => sport.name);
  ...
  const loadEligiblePlayers = async () => {
    ...
    try {
      ...
      for (const doc of docs) {
        ...
        const allUserSports = list.map((entry: string) => {
          const [sp, sk] = entry.toLowerCase().split(':');
          return { sport: sp, skill: sk };
        });

        const isEligible = list.some((e: string) =>
            ...
            {eligiblePlayers.map((player) => (
              ...
              >
                ...
                <div className="flex flex-col gap-2 mb-3">
                  {player.sports.map((item: any, sIdx: number) => (
