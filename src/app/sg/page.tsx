...
const StatCard = ({ heading, value, index, icon: Icon }: { heading: string; value: string | number; index: number; icon: any }) => (
    <motion.div
...
const GrowthCard = ({ index, qrCode, userId }: { index: number; qrCode: string | undefined; userId: string | undefined }) => {
...
const SessionHistoryCard = ({ index, history = [] }: { index: number; history: any[] }) => (
    <motion.div
...
function ChooseActivity({ qrCode }: { qrCode: string | null }) {
    return (
        <motion.div
