...
import { createMatch, deleteMatch, getMatches, updateMatch } from '@/actions/matches';
...
export default function ScoringAdmin() {
    ...
    const fetchMatches = async () => {
        try {
            ...
            if (res.success) setMatches(res.data.documents);
        }
        catch (err) {
            console.error(err);
        }
    ...
    const handleUpdate = async (id: string, data: { score1: string; score2: string; status: string }) => {
        try {
            ...
            if (res.success) {
                ...
                alert(res.error || "Failed to update match");
            }
        }
        catch (err) {
            console.error(err);
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            if (res.success) {
                ...
                alert(res.error || "Failed to delete match");
            }
        }
        catch (err) {
            console.error(err);
...
interface MatchRowProps {
    ...
    onEdit: () => void;
    onCancel: () => void;
    onUpdate: (id: string, data: { score1: string; score2: string; status: string }) => void;
    onDelete: (id: string) => void;
}

function MatchRow({ match, isEditing, onEdit, onCancel, onUpdate, onDelete }: MatchRowProps) {
